...
nondecreasing (x : xs) = (x <= head xs) && nondecreasing xs
...
runLengths [] = []
runLengths (x:xs) = (x,n+1):runLengths new_xs
                  where
                        n = length (takeWhile (==x) xs)
                        (_,new_xs) = splitAt n xs
...
incCount n arr
      | n `elem` [c | (c, _) <- arr] = incCountHelper n arr
      | null arr = [(n, 1)]
      | otherwise = arr ++ [(n, 1)]

incCountHelper :: Eq a => a -> [(a, Int)] -> [(a, Int)]
incCountHelper n arr
      | null arr = []
      | n == char = (n,cnt+1):incCountHelper n xs
      | otherwise = x:incCountHelper n xs
      where
            (x:xs) = arr
            (char, cnt) = x
...
sorts arr1 arr2
      | nondecreasing arr1 = sortsHelper (runLengths arr1) arr2
      | otherwise = False

sortsHelper :: Ord a => [(a, Int)] -> [a] -> Bool
sortsHelper [] [] = True
sortsHelper arr1 arr2 = arr1 == foldr incCount [(c,0) | (c,_)<-arr1] arr2
